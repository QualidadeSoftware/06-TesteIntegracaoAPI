...
| `POST` | `/reset` | Recria os posts iniciais (request auxiliar; útil entre execuções) |
...
Os dados ficam em memória. Requests como `PUT`, `PATCH` e `DELETE` alteram os posts até que a API seja reiniciada ou até que você chame `POST /reset`. Antes de repetir a coleção da atividade, rode `POST /reset` para recriar os posts iniciais.
